...
const INTAKE_URL = "https://turboresponsehq.ai/creator/start?source=zakhy_turbo_chatbot";
...
export default function TurboChatbot() {
      ...
      {open && (
        <section className="turbo-panel" aria-label="Turbo chatbot">
          ...
          <div className="turbo-messages">
            {messages.map((message, index) => (
              <div key={`${message.text}-${index}`} className={`turbo-message turbo-message--${message.from}`}>
                ...
                {message.cta && <a className="turbo-cta" href={INTAKE_URL}>{message.cta}<ArrowUpRight size={15} /></a>}
              </div>
            ))}
          ...
          <footer className="turbo-panel__footer">
            ...
            <a href={INTAKE_URL} className="turbo-intake-link">Open Creator Intake <ArrowUpRight size={14} /></a>
          </footer>
        </section>
      )}
