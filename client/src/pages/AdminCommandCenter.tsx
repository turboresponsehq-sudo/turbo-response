...
function HomeTab({ onNavigate }: { onNavigate: (tab: Tab) => void }) {
  ...
  const { data: driveConfig, error: driveConfigError } = trpc.googleDrive.checkConfig.useQuery();
  ...
  const { data: driveIngestion, error: driveIngestionError } = trpc.googleDrive.ingestionStatus.useQuery(undefined, { enabled: driveConfig?.configured === true, retry: false });
  const beginDriveOAuth = trpc.googleDrive.beginOAuth.useMutation({
  ...
  const uniqueChatLeads = (chatLeads || []).filter((lead: any) => !lead.email || !intakeEmails.has(lead.email.toLowerCase()));
  const liveLeads = [
    ...(intakeLeads || []).map((lead: any) => ({ id: `intake-${lead.id}`, name: lead.fullName, source: lead.source || 'intake', status: lead.status })),
    ...uniqueChatLeads.map((lead: any) => ({ id: `chat-${lead.id}`, name: lead.name, source: 'voice/chat intake', status: lead.status })),
  ].slice(0, 4);
  const attention = [
    ...
    ...(operationalSummary?.unreadMessageCount ? [{ label: `${operationalSummary.unreadMessageCount} unread client message${operationalSummary.unreadMessageCount === 1 ? '' : 's'}`, detail: 'existing case communication needs review', tab: 'workspaces' as Tab, tone: '#f59e0b' }] : []),
  ...
  return <div>
    ...
    <div style={{ ...styles.grid4, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', marginTop: '16px' }}>
      ...
      {card('Voice Intakes', chatLeadsError ? <p style={{ color: '#fca5a5', margin: 0 }}>Voice/chat intake status is unavailable.</p> : chatLeads?.length ? <div>{chatLeads.slice(0, 4).map((lead: any) => <div key={lead.id} style={{ padding: '9px 0', borderBottom: '1px solid #1e293b' }}><strong>{lead.name}</strong><div style={{ fontSize: '12px', color: '#94a3b8' }}>{lead.category || 'voice/chat'} · {lead.status}</div></div>)}</div> : <p style={{ color: '#94a3b8', margin: 0 }}>No voice or chat intakes recorded yet.</p>)}
    </div>
    <div style={{ ...styles.grid4, gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
      {card('Documents & Sync', kbError || operationalSummaryError ? <p style={{ color: '#fca5a5' }}>Document and sync status is unavailable.</p> : <div><strong>{kbStats?.total ?? 0} Knowledge Base documents</strong><p style={{ color: '#94a3b8', margin: '8px 0 0' }}>{pendingSync?.length ?? 0} pending xAI sync · {operationalSummary?.caseDocumentCount ?? 0} legacy case documents · {operationalSummary?.fileUploadCount ?? 0} uploaded files</p><p style={{ color: '#94a3b8', margin: '8px 0 0' }}>{operationalSummary?.caseAnalysisCount ?? 0} legacy case analyses · {operationalSummary?.aiBriefCount ?? 0} saved AI Briefs · {operationalSummary?.workspaceAiAnalysisCount ?? 0} workspace AI analyses</p><p style={{ color: '#94a3b8', margin: '8px 0 0' }}>Google Drive: {driveConfig?.configured ? driveFilesError ? 'file availability unavailable' : `${driveFiles?.files.length ?? 0} recent folder file${(driveFiles?.files.length ?? 0) === 1 ? '' : 's'} available` : driveConfig?.oauth?.clientConfigured ? 'OAuth connection required' : 'OAuth client not configured'}</p><p style={{ color: driveIngestionError ? '#fca5a5' : '#94a3b8', margin: '8px 0 0' }}>Drive ingestion: {driveIngestionError ? 'status unavailable' : !driveIngestion?.run ? 'not started' : `${driveIngestion.discovered} discovered · ${driveIngestion.imported} imported · ${driveIngestion.pending} pending · ${driveIngestion.failed + driveIngestion.unavailable} failed · last sync ${driveIngestion.lastSync ? new Date(driveIngestion.lastSync).toLocaleString() : 'not recorded'}`}</p>{!driveConfig?.configured && driveConfig?.oauth?.clientConfigured ? <button style={{ ...styles.btn('primary'), marginTop: '10px' }} onClick={() => beginDriveOAuth.mutate()} disabled={beginDriveOAuth.isPending}>{beginDriveOAuth.isPending ? 'Opening Google…' : 'Connect Google Drive'}</button> : null}{beginDriveOAuth.error ? <p style={{ color: '#fca5a5', margin: '8px 0 0' }}>Could not start the secure Google connection. Refresh and retry.</p> : null}</div>)}
      {card('Follow-ups & Tasks', tasks?.length ? <div>{tasks.slice(0, 5).map((task: any) => <div key={task.id} style={{ padding: '8px 0', borderBottom: '1px solid #1e293b' }}><strong>{task.title}</strong><div style={{ fontSize: '12px', color: '#94a3b8' }}>{task.priority} · {task.dueDate || 'no due date'}</div></div>)}</div> : operationalSummary?.unreadMessageCount ? <p style={{ color: '#fbbf24', margin: 0 }}>{operationalSummary.unreadMessageCount} unread client message{operationalSummary.unreadMessageCount === 1 ? '' : 's'} need review.</p> : <p style={{ color: '#94a3b8' }}>No open follow-ups recorded.</p>, { label: 'Tasks', tab: 'tasks' })}
    </div>
    <div style={{ ...styles.grid4, gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
      {card('System Health', <div style={{ fontSize: '13px', color: '#94a3b8' }}><div>Website/API: {serviceHealth?.ok ? 'available' : 'status unavailable'}</div><div style={{ marginTop: '7px' }}>Google Drive: {driveConfig?.configured ? 'configured' : 'not configured'} · CRM: {operationalSummary?.hubSpotConfigured ? 'HubSpot configured' : 'HubSpot not configured'} · xAI status: {pendingSyncError ? 'unavailable' : 'tracked through pending sync'}</div><div style={{ marginTop: '7px' }}>Voice agent: no activity-feed API is currently exposed.</div></div>)}
      {card('Marketing & Activity', signals?.length ? <div>{signals.slice(0, 4).map((signal: any) => <div key={signal.id} style={{ padding: '8px 0', borderBottom: '1px solid #1e293b' }}><strong>{signal.companyName}</strong><div style={{ fontSize: '12px', color: '#94a3b8' }}>{signal.signalType || 'signal'} · {signal.sourceType || 'source not recorded'}</div></div>)}</div> : operationalSummary?.businessIntakeCount ? <p style={{ color: '#94a3b8', margin: 0 }}>{operationalSummary.businessIntakeCount} business intake record{operationalSummary.businessIntakeCount === 1 ? '' : 's'} · {operationalSummary.businessSubmissionCount} submitted business record{operationalSummary.businessSubmissionCount === 1 ? '' : 's'}.</p> : <p style={{ color: '#94a3b8' }}>No lead-generation signals recorded.</p>, { label: 'Signals', tab: 'signals' })}
    </div>
  </div>;
}
