...
async function ensureAiLearningStorage() {
  if (storageReady) return storageReady;
  storageReady = (async () => {
    const db = await getDb();
    if (!db) throw new Error("Intake storage is unavailable");
    await db.execute(sql`CREATE TABLE IF NOT EXISTS ai_learning_intakes (
    ...
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_ai_learning_intakes_submitted ON ai_learning_intakes (submitted_at)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_ai_learning_intakes_email ON ai_learning_intakes (email)`);
    await db.execute(sql`INSERT INTO schema_migrations (migration_name)
      SELECT '0020_create_ai_learning_intakes'
      WHERE NOT EXISTS (SELECT 1 FROM schema_migrations WHERE migration_name = '0020_create_ai_learning_intakes')`);
  })().catch((error) => {
    storageReady = null;
    throw error;
  });
...
aiLearningRouter.post("/ai-learning-intake", async (req: any, res) => {
  ...
  if (!name || !email || !experience || !learningInterests.length || !goal || !learningStyle) return res.status(400).json({ error: "Please complete the required intake fields." });
  ...
  try {
    ...
    sendOwnerNotification("New AI Learning Intake", [
      `Name: ${name}`, `Email: ${email}`, `Phone: ${phone || "Not provided"}`, `Experience: ${experience}`,
      `Wants to learn: ${learningInterests.join(", ")}`, `Goal: ${goal}`, `Learning style: ${learningStyle}`,
      `Submitted: ${submission?.submitted_at || new Date().toISOString()}`,
    ], "https://turboresponsehq.ai/admin/ai-learning-intakes").catch((error) => console.warn("[AI Learning] Owner notification failed:", error));
