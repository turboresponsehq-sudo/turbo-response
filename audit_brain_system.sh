...
SETUP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_BASE/api/brain/setup" -H "x-access-token: REDACTED_RETIRED_ACCESS_TOKEN")
LIST_CODE=$(curl -s -o /dev/null -w "%{http_code}" "$API_BASE/api/brain/list" -H "x-access-token: REDACTED_RETIRED_ACCESS_TOKEN")
