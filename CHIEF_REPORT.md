# TURBO RESPONSE HQ - CHIEF'S REFERENCE REPORT
...
- **URL:** https://turboresponsehq.ai/admin/login
...
- **Service Email:** turboresponsehq@gmail.com
- **SMTP User:** turboresponsehq@gmail.com
...
### GitHub Repository
- **URL:** https://github.com/turboresponsehq-sudo/turbo-response
- **Branch:** main
...
- **Consumer Login:** https://turboresponsehq.ai/client/login
- **Business Login:** https://turboresponsehq.ai/business/login
...
1. Client fills out form at https://turboresponsehq.ai
2. Case saved to database with unique case number (e.g., TR-1234567890)
3. Admin receives email notification
4. Client receives confirmation email with case number and portal link

### Client Logs Into Portal
1. Client enters email + case number
2. System generates 6-digit verification code
3. Code sent to client's email
4. Client enters code → authenticated for 24 hours
5. Can view case, send messages, upload documents

### Admin Manages Cases
1. Login at /admin/login with email/password
2. View all cases in dashboard
3. Click case to see details
4. Reply to messages → **Client gets email notification**
5. Update status → **Client gets email notification**
6. View/download documents in gallery

### Document Upload Flow
1. Client uploads document via portal
2. File sent to backend API
3. Backend uploads to S3 storage
4. S3 URL saved to database
5. **Admin receives email notification**
6. Document appears in admin gallery with thumbnail
...
4. Server restarts and updates password hash

**Current password:** Turbo1234!

### If Admin Login Fails
1. Check password is exactly: `Turbo1234!` (capital T, exclamation at end)
2. Check email is: `turboresponsehq@gmail.com`
3. Check Render logs for seed script success message
4. If still failing, trigger manual Render redeploy to run seed script

### If Emails Not Sending
1. Check Render environment variables:
   - `EMAIL_USER` = turboresponsehq@gmail.com
   - `EMAIL_PASSWORD` = [Gmail App Password]
   - `ADMIN_EMAIL` = collinsdemarcus4@gmail.com
2. Check Gmail App Password is still valid
3. Check Render logs for email errors
4. Test by submitting a new case

### If Client Can't Login
1. Verify case exists in database
2. Check email matches case email exactly
3. Check case number format (TR-XXXXXXXXXX)
4. Check verification code email was sent
5. Code expires after 10 minutes - request new code

### If Documents Not Uploading
1. Check file size (max 10MB)
2. Check file type (PDF, JPG, PNG, HEIC only)
3. Check S3 credentials in Render environment
4. Check backend logs for upload errors

### If Android Issues
- **Verified working on:** Samsung Internet browser
- **Login credentials:** turboresponsehq@gmail.com / Turbo1234!
...
2. **Admin internal notes** - Private notes visible only to admin
3. **Bulk status updates** - Update multiple cases at once
4. **Advanced filtering** - Filter by date range, status, category
5. **Client satisfaction survey** - Auto-send after case closed
...
2. Commit to GitHub: `git push github main`
...
2. Select "turbo-response-live" service
...
### Technical Issues
- **Manus AI Support:** https://help.manus.im
- **Render Support:** https://render.com/docs
- **TiDB Support:** https://tidbcloud.com/console

### Service Providers
- **Domain:** turboresponsehq.ai (managed via Render)
- **Email:** Gmail SMTP (turboresponsehq@gmail.com)
...
**Admin Dashboard:**
- [ ] Can login with turboresponsehq@gmail.com / Turbo1234!
...
1. Open https://turboresponsehq.ai/admin/login
2. Login with turboresponsehq@gmail.com / Turbo1234!
3. Verify dashboard loads and shows cases
4. Test one message reply to verify client notification
5. Test one status update to verify client notification
6. Check email inbox for any error notifications
...
**TiDB:** https://tidbcloud.com/console
