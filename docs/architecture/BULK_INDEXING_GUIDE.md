...
1. **Chunk document** into 800-token segments with 100-token overlap

1. **Generate embeddings** using OpenAI text-embedding-3-small (1536 dimensions)

1. **Store chunks** with embeddings in `brain_chunks` table

1. **Update status** to `indexed` with chunk count and timestamp
...
1. **PDF format** - Some PDFs are scanned images (no extractable text )

1. **OpenAI API limits** - Check if you hit rate limits
...
1. **Relevant context** is injected into agent responses

1. **Source citations** are included in responses
...
1. ✅ Test RAG retrieval with sample questions

1. ✅ Monitor Turbo agent responses for Brain context usage

1. ✅ Add more documents as needed (auto-indexed on upload)
